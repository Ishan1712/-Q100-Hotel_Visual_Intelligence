    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between animate-fade-in-up">
        ...
        <div className="flex items-center gap-2">
          <button className="flex items-center gap-1.5 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">
            <Printer size={15} />
            Print
          </button>
          <button className="flex items-center gap-1.5 px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-600 hover:bg-slate-50 transition-all">
          ...
          <button className="flex items-center gap-1.5 px-4 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-200">
            <Mail size={15} />
            Email Report
          </button>
        </div>
      ...
      {/* Cost Savings Card — FIXED layout */}
      <div className="glass-card rounded-2xl p-6 relative overflow-hidden animate-fade-in-up stagger-2">
      ...
      <div className="glass-card rounded-2xl p-6 animate-fade-in-up stagger-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {catchesOfTheDay.map((c, i) => {
            ...
            return (
              <div key={i} className={`p-5 rounded-2xl border ${severityStyle[c.severity]}`}>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-lg font-black text-slate-900">Room {c.room}</span>
                  <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase ${badgeStyle[c.severity]}`}>{c.severity}</span>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="glass-card rounded-2xl p-6 animate-fade-in-up stagger-4">
          <h2 className="text-xs font-black text-slate-400 uppercase tracking-[0.3em] mb-5">Trend vs. Last Week</h2>
        ...
        <div className="glass-card rounded-2xl p-6 animate-fade-in-up stagger-5">
          <h2 className="text-xs font-black text-slate-400 uppercase tracking-[0.3em] mb-5">Guest Satisfaction Correlation</h2>
