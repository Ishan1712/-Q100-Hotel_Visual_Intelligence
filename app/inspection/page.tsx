...
export default function InspectionFlowPage() {
  ...
  const viewedCheckpoint =
    viewingCheckpointId !== null
      ? checkpoints.find((c) => c.id === viewingCheckpointId) || null
      : null;

  const viewedInspectionImage =
    viewedCheckpoint ? toImageSrc(capturedImages[viewedCheckpoint.id]) : null;
  ...
  useEffect(() => {
    if (selectedRoom && Object.keys(capturedImages).length > 0) {
      localStorage.setItem(
        `q100_room_captured_images_${selectedRoom.number}`,
        JSON.stringify(capturedImages)
      );
    }
  }, [capturedImages, selectedRoom]);
  ...
  useEffect(() => {
    if (phase === 'video') {
      ...
      const captureTimers = [
        ...
        return setTimeout(() => {
          triggerVideoCapture(c.id, c.status as 'pass' | 'fail');
        }, c.time * 1000);
      });
  ...
  const renderRoomSelection = () => (
    <div className="p-4 md:p-6 space-y-6 md:space-y-8 animate-fade-in h-full overflow-y-auto no-scrollbar bg-[var(--bg-gradient)] rounded-2xl md:rounded-[2rem]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 pb-10 items-start">
        {rooms
          .filter(r =>
            r.number.toLowerCase().includes(roomSearchQuery.toLowerCase()) ||
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  const savedCapturedImages = localStorage.getItem(`q100_room_captured_images_${room.number}`);

                  if (savedResults) {
                    setResults(JSON.parse(savedResults));
                    setCapturedImages(savedCapturedImages ? JSON.parse(savedCapturedImages) : {});
                    setPhase("report");
                  } else {
                    setResults({});
                    setCapturedImages({});
              ...
              <div className="flex gap-3 overflow-x-auto no-scrollbar">
                {captureStrip.map((cap, i) => (
                  <div key={i} className="flex-shrink-0 relative">
                    ...
                    <div className={`absolute -top-2 -right-2 w-6 h-6 rounded-full flex items-center justify-center shadow-lg border-2 border-slate-900 ${cap.status === 'pass' ? "bg-emerald-500" : "bg-rose-500"
                      }`}>
                      {cap.status === 'pass'
  ...
  const renderReport = () => (
    <div className="p-4 md:p-6 space-y-6 md:space-y-8 animate-fade-in h-full overflow-y-auto no-scrollbar bg-white rounded-2xl md:rounded-3xl shadow-2xl border border-slate-100">
      <div className="flex flex-col sm:flex-row justify-between items-center bg-gradient-to-r from-blue-50/80 to-indigo-50/80 p-4 md:p-6 rounded-2xl md:rounded-[2rem] border border-blue-100/30 gap-4 shadow-sm">
        <div className="text-center sm:text-left">
          ...
          <div className="flex items-center justify-center sm:justify-start gap-2 mt-2">
            <span className="bg-slate-900 text-white px-2.5 py-1 rounded-lg text-[8px] md:text-[9px] font-bold uppercase tracking-widest shadow-sm">
        ...
        <button
          onClick={() => {
            ...
            localStorage.removeItem(`q100_room_captured_images_${selectedRoom?.number}`);
      ...
      <div className="space-y-4 pb-8">
        <h3 className="text-[8px] md:text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100 pb-2">
          Checkpoint Analysis
        </h3>
              ...
              >
                ...
                {(status === 'pass' || status === 'fail') && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setViewingCheckpointId(cp.id);
                    }}
                    className="absolute top-3 right-3 p-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg transition-all active:scale-95 z-10"
                    title="View Images"
                  >
                    <Camera size={16} />
                  </button>
                ...
                <span className="text-4xl md:text-5xl transition-transform group-hover:scale-110 group-hover:rotate-6">
                  {cp.icon}
                </span>

                <span className="text-[9px] md:text-[10px] font-black text-slate-900 uppercase tracking-widest text-center w-full">
                  {cp.name}
                </span>
                ...
                <div className="w-full mt-auto">
                  {status === 'fail' ? (
                    <div className="flex flex-row gap-2 w-full">
                      <div className="flex-1 py-2.5 bg-rose-50 text-rose-600 border border-rose-100 rounded-xl text-[8px] md:text-[9px] font-bold uppercase tracking-widest text-center flex items-center justify-center">
                        Failed
                      </div>
                      <button
                        onClick={(e) => {
                        ...
                        className="flex-1 py-2.5 bg-[#ff1b51] text-white rounded-xl shadow-md shadow-rose-500/30 font-bold text-[8px] md:text-[9px] uppercase tracking-widest flex items-center justify-center hover:scale-105 active:scale-95 transition-all"
                      >
                        Re-Capture
                      </button>
                    </div>
  ...
  return (
    <div className="relative w-full h-[calc(100vh-8rem)] md:h-[calc(100vh-10rem)] min-h-[500px] md:min-h-[600px] bg-slate-50 rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl font-sans border border-slate-100">
        ...
        <div className="absolute inset-0 bg-white/95 backdrop-blur-3xl z-[300] flex flex-col justify-end animate-fade-in p-4 md:p-6">
          <div className="max-w-xs mx-auto w-full pb-4 md:pb-6">
            <div className="flex justify-between items-center mb-4 md:mb-6 px-2">
              ...
              <button onClick={() => setShowKeypad(false)} className="bg-slate-100 p-2 md:p-3 rounded-xl text-slate-400 hover:text-slate-900 leading-none shadow-sm">
                <X size={20} />
              </button>
            </div>
      ...
      {viewedCheckpoint && (
        <div className="absolute inset-0 z-[400] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in">
          <div className="w-full max-w-5xl bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 md:p-5 border-b border-slate-100">
              <div>
                <h3 className="text-lg md:text-xl font-black text-slate-900 tracking-tight">
                  {viewedCheckpoint.name}
                </h3>
                <p className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-1">
                  Master and inspection view
                </p>
              </div>

              <button
                onClick={() => setViewingCheckpointId(null)}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 md:p-5">
              <div className="bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden">
                ...
                <div className="aspect-[4/3] flex items-center justify-center bg-slate-100">
                  {viewedCheckpoint.masterImg ? (
                    <img
                      src={viewedCheckpoint.masterImg}
                      alt={`${viewedCheckpoint.name} master`}
                      className="w-full h-full object-contain bg-white"
                    />
                  ) : (
                    <div className="text-slate-400 text-sm font-semibold">No master image</div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden">
                ...
                <div className="aspect-[4/3] flex items-center justify-center bg-slate-100">
                  {viewedInspectionImage ? (
                    <img
                      src={viewedInspectionImage}
                      alt={`${viewedCheckpoint.name} inspection`}
                      className="w-full h-full object-contain bg-white"
                      onError={() => {
                        console.error("Inspection image failed to load:", viewedInspectionImage);
                      }}
                    />
                  ) : (
                    <div className="text-slate-400 text-sm font-semibold">
                      No inspection image captured
                    </div>
                  )}
                </div>
              </div>
